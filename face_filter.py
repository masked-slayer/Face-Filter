...
while True:
    _, frame = cap.read()
    nose_mask.fill(0)
    gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) # convert image to gray scale

    faces = detector(frame)  # load the captured frame to face detctor
    for face in faces:
        landmarks = predictor(gray_frame, face) # load the detected face in 68-landmark predictor 
        ...
        # Re-sizing our dog-nose filter image
        nose_width = int(hypot(left_nose[0] - right_nose[0],left_nose[1] - right_nose[1]) * 4.7)
        nose_height = int(nose_width * 0.9)
        nose_dog = cv2.resize(nose_image, (nose_width, nose_height))

         # location of our original nose
       ...
       # Adding the dog-nose filter on our original nose
